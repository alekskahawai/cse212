...
public static class SetsAndMapsTester
{
    ...
    ///
    /// As a special case, if the letters are the same (example: 'aa') then it would not match anything else (remember the assumption above that there were no duplicates) and therefore should not be displayed.
    /// </summary>
    /// <param name="words">An array of 2-character words (lowercase, no duplicates)</param>
    private static void DisplayPairs(string[] words)
    {
        // initiate a new set to keep track of values seen
        var valueSeen = new HashSet<string>();

        // iterate through all values in 'words' collection
        foreach (var word in words)
        {
            // reverse each value
            var tail = word[0].ToString();
            var pair = word[1].ToString();
            pair += tail;
            // var pair = word.ToCharArray().Reverse();
            // var pair = new string(word.ToCharArray().Reverse().ToArray());

            // check if we already saw the 'pair' value 
            // To display the pair correctly use something like:
            // Console.WriteLine($"{word} & {pair}");
            // Each pair of words should displayed on its own line.
            // if (valueSeen.Contains(word.Reverse<string>())
            if (valueSeen.Contains(pair))
                Console.WriteLine($"{word} & {pair}");

            // populate the set to keep track of values seen
            valueSeen.Add(word);
        }
    }
    ...
    // Problem 2: Degree Summary
    /// <summary>
    /// Read a census file and summarize the degrees (education) earned by those contained in the file.  The summary should be stored in a dictionary where the key is the degree earned and the value is the number of people that have earned that degree.  The degree information is in the 4th column of the file. There is no header row in the file.
    /// </summary>
    ...
    private static Dictionary<string, int> SummarizeDegrees(string filename)
    {
        ...
        foreach (var line in File.ReadLines(filename))
        {
            ...
            // initiate a variable to get the degree's name in the line fields[3]
            var degreeName = fields[3];
            //var count = 1;

            // check 'degrees' dictionary
            // if contains the degree, increase counter by 1
            if (degrees.ContainsKey(degreeName))
                //degrees[degreeName] += count;
                degrees[degreeName] += 1;

            // else add the degree to the dictionary, set counter to 1
            else
                //degrees[degreeName] = count;
                degrees[degreeName] = 1;
        }
    ...
    /// 
    /// Reminder: You can access a letter by index in a string by using the [] notation.
    /// </summary>
    ...
    // Problem 5: Earthquake
    /// <summary>
    /// This function will read JSON (Javascript Object Notation) data from the United States Geological Service (USGS) consisting of earthquake data. The data will include all earthquakes in the current day.
    /// JSON data is organized into a dictionary. After reading the data using the built-in HTTP client library, this function will print out a list of all earthquake locations ('place' attribute) and magnitudes ('mag' attribute). Additional information about the format of the JSON data can be found at this website: https://earthquake.usgs.gov/earthquakes/feed/v1.0/geojson.php  </summary>
    private static void EarthquakeDailySummary()
    {
        ...
        // TODO:
        // 1. Add code in FeatureCollection.cs to describe the JSON using classes and properties on those classes so that the call to Deserialize above works properly.
